...
create_univ () {
    if [ -e "pdfium-darwin-x64$1.tgz" ] && [ -e "pdfium-darwin-arm64$1.tgz" ]; then
        echo "Extracting x64..."
        mkdir -p $STAGE_DIR/x64
        cd $STAGE_DIR/x64
        tar xf $SRC_DIR/pdfium-darwin-x64$1.tgz

        echo "Extracting arm64..."
        mkdir -p $STAGE_DIR/arm64
        cd $STAGE_DIR/arm64
        tar xf $SRC_DIR/pdfium-darwin-arm64$1.tgz
        ...
        cd $STAGE_DIR/univ
        tar cf "$SRC_DIR/pdfium-darwin-universal$1.tgz" -- *
